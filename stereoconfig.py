...
class stereoCamera1(object):
    def __init__(self):
        # 左相机内参
        self.cam_matrix_left = np.array([[721.5377, 0.0000, 609.5593],
                                         [0.0000, 721.5377, 172.8540],
                                         [0.0000, 0.0000, 1.0000]])
        # 右相机内参
        self.cam_matrix_right = np.array([[721.5377, 0.0000, 609.5593],
                                         [0.0000, 721.5377, 172.8540],
                                         [0.0000, 0.0000, 1.0000]])

        # 左右相机畸变系数:[k1, k2, p1, p2, k3]
        self.distortion_l = np.array([[0.0000, 0.0000, 0.0000,
                                       0.0000, 0.0000]])
        self.distortion_r = np.array([[0.0000, 0.0000, 0.0000,
                                       0.0000, 0.0000]])

        # 旋转矩阵
        self.R = np.array([[0.0000, 0.0098, -0.0074],
                           [-0.0098, 0.9999, -0.0043],
                           [0.0074, 0.0043, 9.999]])

        # 平移矩阵
        self.T = np.array([[-540.0000], [0.0000], [0.0000]])
